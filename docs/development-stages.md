...
Бот принимает фото, управляет состояниями, реализован режим "эталон + дефект", запускается обработка в фоне.
...
- [x] Добавить зависимость `gocv.io/x/gocv` в `go.mod`
- [x] Создать папку `internal/infrastructure/vision/`
- [x] Реализовать `detector.go` — `GoCVDetector` (реализация `DefectDetector`)
  - [x] Загрузка изображения
  - [x] Преобразование в grayscale
  - [x] Размытие (GaussianBlur)
  - [x] Детекция отличий (Diff) между эталоном и текущим фото
  - [x] Поиск контуров
  - [x] Фильтрация по площади и форме
- [x] Реализовать подсветку дефектов на изображении
- [x] Добавить build-tag `gocv` и заглушку без OpenCV
- [x] Обновить Dockerfile для сборки с OpenCV
- [ ] Написать тесты с тестовыми изображениями

### Результат

Бот сравнивает эталон и дефект, находит отличия и возвращает фото с подсветкой.
...
- [x] Реализовать `UserService` и `InspectionService`
- [x] Перенести логику оркестрации из бота в сервис
- [x] Подключить сервисы через контейнер
...
| 3. Telegram-бот | ✅ Готово | Режим "эталон + дефект" |
| 4. Компьютерное зрение | ✅ Готово | Diff + фильтрация + подсветка |
| 5. ИИ-описание | ⏳ Не начат | — |
| 6. Application Service | ✅ Готово | UserService + InspectionService |
